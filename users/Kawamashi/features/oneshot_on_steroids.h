 ...
 * KC_NO must be used if the one-shot key triggers no modifier.
...
// Function to deactivate all one-shot keys
void clear_oneshot(void);

// Custom one-shot keys implementation that doesn't rely on timers. If a mod is
